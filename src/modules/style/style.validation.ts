import { z } from 'zod';
import { SUPPORTED_LANGUAGES } from './style.types';
import { paginationQuerySchema } from '../../shared/pagination/pagination.validation';

const slugRegex = /^[a-z0-9-]+$/;
const coverImageRegex = /^content\/styles\/.*$/;
...
export const createStyleSchema = z.object({
  body: z.object({
    ...
    coverImageKey: z
      .string()
      .regex(coverImageRegex, 'Cover image key must start with content/styles/')
      .optional(),
...
export const styleSlugParamSchema = z.object({
  params: z.object({
    slug: z
      .string()
      .regex(slugRegex, 'Slug must be lowercase and contain only letters, numbers, and hyphens'),
